  ...
  try {
    ...
    for (const product of newProducts) {
      ...
      if (product.productType === "variations") {
        ...
        if (subs.length > 0) {
          const sub_options = subOptions.filter((item) =>
            subs
              .map((it) => String(it._id))
              .includes(String(item.sub_product_id))
          );
          if (sub_options.length > 0) {
            await embedingProduct(product, subs, sub_options);
          }
        }
      }
    }
...
export const testAPI2 = async (req: Request, res: Response) => {
  try {
    ...
    const result = await qdrantClient.search("products", {
      vector,
      filter: {
        should: [
          {
      ...
      if (!object.new) {
        ...
        if (points && points.length > 0) {
          if (object.query.productType === "simple") {
            const products = await qdrantClient.retrieve("products", {
              ids: points,
              with_payload: true,
              with_vector: false,
            });
            ...
            return await promptProduct(
              ...
              chatModel,
              object.intent,
              products.map((item) => item.payload)
            );
          } else {
            const products = await qdrantClient.retrieve("sub-products", {
              ids: points,
              with_payload: true,
              with_vector: false,
            });
        ...
        const products = await getProducts(input, object.query, req);
        await promptProduct(
          req,
          res,
          input,
...
const getProducts = async (input: string, object: any, req: Request) => {
  try {
    const response = await gemAI.models.embedContent({
      model: "gemini-embedding-001",
      contents: input,
